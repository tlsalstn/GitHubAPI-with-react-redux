...
function LoginContainer() {
    const { authState, getTokenData } = useAuth()
    ...
    useEffect(() => {
        ...
        if(params["code"] && params["state"]) {
            const newUrl = location.href.split("?code=")
            history.pushState({}, "", newUrl[0])
            getTokenData(params["code"], params["state"])
        }
    }, [])
